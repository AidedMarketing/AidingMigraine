...
1. ~~**Symptoms & aura tracking**~~ — ✅ **Shipped in v4.2.0.** Controlled ICHD-3-aligned list (nausea, light/sound/smell sensitivity, visual/sensory/speech aura, neck pain, dizziness, blurred vision, fatigue) captured on the log form + active-episode card, editable in both edit flows, shown in history/calendar/details, ranked in a new analytics card, and exported to CSV/PDF/JSON. Future extension: aura *duration/timing* phases (prodrome/postdrome) — see item 6.
2. ~~**Trigger tracking UI**~~ — ✅ **Shipped in v4.3.0.** Controlled 19-item trigger chip list on the log form + active-episode card, editable in both edit flows, shown in history/calendar/details, ranked in a "Common Triggers" analytics card, and exported to CSV/PDF/JSON. ✅ **Trigger×weather correlation shipped in v4.7.0** — a "Triggers × weather" analytics card validates each self-reported trigger (esp. "Weather change") against the measured per-episode pressure activity vs the user's baseline.
3. ~~**MIDAS + HIT-6 questionnaires**~~ — ✅ **Shipped in v4.4.0.** Both instruments are taken in-app from a Care-page "Disability assessments" card, scored/graded, persisted locally, included in JSON backup/restore, and embedded in the PDF doctor report.
4. ~~**Surface the ML/risk predictions**~~ — ✅ **Shipped in v4.6.0.** A "Today's outlook" card at the top of Today surfaces the 0–10 daily risk score (meter + label), the pressure trend/24h change, the personal ML migraine-probability (only when `mlModel.enabled`), and the most imminent forecast alerts (`generatePredictiveAlerts`) with recommendations. Hidden until weather tracking has a current reading. Future extension: consolidate the in-log-card `#weather-forecast-indicator` into this card; optional push alert on high-risk forecast.
5. ~~**One-tap "attack mode" logging**~~ — ✅ **Shipped in v4.5.0.** A "Start attack now" button at the top of the log card starts an episode with a single tap and auto-captured weather; pain, symptoms, triggers, and relief are refined later on the active-episode card. Pain is left unset (null) rather than defaulted, so in-progress episodes don't pollute averages, and a pain level is required only at end. Future extension: an in-attack dark/minimal UI mode.
6. ~~**Prodrome/postdrome phase tracking**~~ — ✅ **Shipped in v4.8.0.** Two controlled-list chip fields: prodrome (warning signs) captured in the log form's Optional Details, postdrome (after-effects) captured in the End-Episode modal. Editable in both edit flows, shown in history/calendar/details ordered before→during→after, ranked in "Common warning signs"/"Common after-effects" analytics cards, and exported to CSV/PDF/JSON. UI tucked into optional/secondary surfaces to protect FTUE.
7. ~~**Head-pain location map + pain quality**~~ — ✅ **Shipped in v4.9.0.** An interactive SVG head map (front + back, ~10 tappable zones) captures `painLocations` with derived laterality (left/right/bilateral/central); a chip multi-select captures `painQuality` (pulsating/pressing/stabbing/burning/dull). Tucked into the collapsed Optional Details + both edit flows, shown in history/calendar/details (read-only map in Episode Details), ranked in two analytics cards, and exported to CSV/PDF/JSON. Keyboard-accessible.
8. ~~**Menstrual/hormonal cycle tracking**~~ — ✅ **Shipped in v4.10.0.** Opt-in (off by default, Settings toggle), device-only period-start logging on a Care-page Cycle card with derived cycle day + next-period estimate, a subtle Today line, and a baseline-honest menstrual-migraine correlation (perimenstrual window day −2..+3) surfaced on the card + PDF report. Period dates never leave the device. **This closes the P2 competitive-feature backlog (items 1–8).**
